...
class CustomEnvironment(Environment):
    ...
    def fitness_single(self):
        # Standard fitness function: player's life minus enemy's life
        ep = self.player.life  # Player's remaining energy
        ee = self.enemy.life   # Enemy's remaining energy

        # Compute the standard fitness
        fit = ep - ee
        return fit
...
def individual_island_run(env, island_population, pop_fit, mutation_rate, exit_local_optimum):
    ...
    # Add new children
    updated_island_population = np.vstack((updated_island_population, child_1_mutated, child_2_mutated))
    ...
    for i in range(ini_g, n_gens):
        ...
        if i % migration_interval == 0 and i != 0:
            ...
            for idx in range(len(world_population)):
                fitnesses = evaluate(env, world_population[idx])
                world_pop_fit[idx] = fitnesses
...
if os.path.exists(experiment_name + '/neuroended'):
    ...
    env = CustomEnvironment(experiment_name=experiment_name,
                            ...
                            level=2,
                            multiplemode="yes",
                            visuals=True)  # Set visuals to True for testing
